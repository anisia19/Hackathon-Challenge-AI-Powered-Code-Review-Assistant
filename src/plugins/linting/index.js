...
async function run(filesToReview) {
    ...
    const command = `npx eslint ${filesString} --format json --no-eslintrc --ignore-path .gitignore || true`;
    ...
    try {
        ...
        return `### Linting Error\n\nStatic analysis could not be performed. Check if \`eslint\` is properly installed and configured in the project dependencies.\nDetails: \`\`\`\n${error.message}\n\`\`\``;
    }
}
